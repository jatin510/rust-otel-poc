...
pub fn init_tracer() -> Result<opentelemetry::sdk::trace::Tracer, opentelemetry::global::Error> {
    ...
    let jaeger_host =
        env::var("OTEL_EXPORTER_JAEGER_AGENT_HOST").unwrap_or_else(|_| "jaeger".to_string());
    let jaeger_port = env::var("OTEL_EXPORTER_JAEGER_AGENT_PORT")
        .unwrap_or_else(|_| "6831".to_string())
        .parse::<u16>()
        .unwrap_or(6831);
    let service_name =
        env::var("OTEL_SERVICE_NAME").unwrap_or_else(|_| "rust-otel-service".to_string());

    println!(
        "Configuring Jaeger exporter with host: {}, port: {}, service: {}",
...
pub fn init_tracing() {
    ...
    Registry::default().with(env_filter).with(telemetry).init();
